...
//验证投注的号码是否正确
func (pd *PlayDlt) Verification() (bool, error) {
	var flags bool = pd.CheckPlaytype()
	if !flags {
	...
	if pd.ticketNum != pd.numLottery.BetNum {
		return false, errors.New("注数计算错误")
	}

	//单注不能超过两万
	if (pd.numLottery.Money / pd.numLottery.Multiple) > 20000 {
	...
	if pd.ticketNum*oneMoney*pd.numLottery.Multiple != pd.numLottery.Money {
